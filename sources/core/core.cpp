...
int
core::Core::getUserCount() const noexcept
{
...
std::string
core::Core::getPrefix(std::string aDelimiter) noexcept
{
    std::string result = "";
    if (mCommand.ifFlagSet(Command::Flag::PREFIX))
    {
        result = mCommand.getArgCell(int(Command::Flag::PREFIX), 0).value();
        result += aDelimiter;
    }
    return result;
}
...
void
core::Core::generateRoboNames() noexcept
{
    int count = getUserCount();
